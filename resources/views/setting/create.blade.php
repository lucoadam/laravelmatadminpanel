...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('setting.store') }}" autocomplete="off" class="form-horizontal" >
            @csrf
            @method('post')
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('key') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('key') ? ' is-invalid' : '' }}" name="key" id="input-key" type="text" placeholder="{{ __('Key') }}" value="{{ old('key') }}" required="true" aria-required="true"/>
                      @if ($errors->has('key'))
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('value') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('value') ? ' is-invalid' : '' }}" name="value" id="input-value" type="text" placeholder="{{ __('Value') }}" value="{{ old('value') }}" required="true" aria-required="true"/>
                      @if ($errors->has('value'))
