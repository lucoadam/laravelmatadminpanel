...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('content.store') }}" autocomplete="off" class="form-horizontal" >
            @csrf
            @method('post')
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                      <textarea rows="5" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" name="title" id="input-title" placeholder="{{ __('Title') }}" value="{{ old('title') }}" required="true" aria-required="true"></textarea>
                      @if ($errors->has('title'))
                  ...
                  @endphp
                              ...
                              <select class="form-control{{ $errors->has('image_id') ? ' is-invalid' : '' }}" name="image_id">
                                  <option selected disabled value="">Image</option>
                  ...
                  @endphp
                              ...
                              <select class="form-control{{ $errors->has('file_id') ? ' is-invalid' : '' }}" name="file_id">
                                  <option selected disabled value="">File</option>
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('type') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('type') ? ' is-invalid' : '' }}" name="type" id="input-type" type="text" placeholder="{{ __('Type') }}" value="{{ old('type') }}" required="true" aria-required="true"/>
                      @if ($errors->has('type'))
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('posted_by') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('posted_by') ? ' is-invalid' : '' }}" name="posted_by" id="input-posted_by" type="text" placeholder="{{ __('Posted_by') }}" value="{{ old('posted_by') }}" required="true" aria-required="true"/>
