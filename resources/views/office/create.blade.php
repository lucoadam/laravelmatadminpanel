...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('office.store') }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('post')

            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">{{ __('Add Office') }}</h4>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('staff_no') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('staff_no') ? ' is-invalid' : '' }}" name="staff_no" id="input-staff_no" type="text" placeholder="{{ __('Staff_no') }}" value="{{ old('staff_no') }}" required="true" aria-required="true"/>
              ...
              <div class="card-footer ml-auto mr-auto">
                <button type="submit" class="btn btn-primary">{{ __('Add Office') }}</button>
              </div>
            </div>
          </form>
        </div>
