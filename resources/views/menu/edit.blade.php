...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('menu.update', $menu) }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('put')
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('icon') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('icon') ? ' is-invalid' : '' }}" name="icon" id="input-icon" type="text" placeholder="{{ __('Icon') }}" value="{{ old('icon', $menu->icon) }}" required="true" aria-required="true"/>
                      @if ($errors->has('icon'))
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('url_type') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('url_type') ? ' is-invalid' : '' }}" name="url_type" id="input-url_type" type="text" placeholder="{{ __('Url_type') }}" value="{{ old('url_type', $menu->url_type) }}" required="true" aria-required="true"/>
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('open_in_new_tab') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('open_in_new_tab') ? ' is-invalid' : '' }}" name="open_in_new_tab" id="input-open_in_new_tab" type="text" placeholder="{{ __('Open_in_new_tab') }}" value="{{ old('open_in_new_tab', $menu->open_in_new_tab) }}" required="true" aria-required="true"/>
